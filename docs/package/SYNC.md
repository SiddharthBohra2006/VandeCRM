...
- ⚠️ **NOTE:** OpenCode edited `WorkListPage.tsx` + `server/src/api/work.js` (Antigravity-owned) for the board/calendar port — see OWNERSHIP flag. Antigravity should review.
- 🔶 **OPEN (priority order, parsed from deep gap analysis):** (1) Dashboard parity (tinted KPI icons, card-pinning modal, weekly-chart tooltips, rich pipeline kanban), (2) Leads table parity (stage pills, avatars, WhatsApp/Call, hot-lead badges, sticky/frozen), (3) Clients table parity (portfolio KPIs, won-value tags, project counts), (4) Lead detail parity (contact strip, Quick Activity Composer, quick-postpone, right-sidebar controls), (5) Client detail parity (summary KPIs, linked work items, documents), then settings work-types editor + automations, work-detail parity, lead-duplicates + work CSV import, 403/404/500, inline-style consolidation.
