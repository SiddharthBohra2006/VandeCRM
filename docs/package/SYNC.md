...
## 2026-09-05 recheck of commit 5a17aef
Production build passed (1915 modules), full TypeScript passed, and all 19 API files plus server entry passed syntax checks. Follow-up fixes applied: restricted dashboard custom fields in stage/attention/definition payloads; cross-workspace lead creation and duplicate operations constrained; Integration clear actions call dedicated APIs; Clients bulk delete uses accessible ConfirmDialog; dashboard Activity option controls the rendered activity feed. Staged TypeScript and targeted invariants passed. Live authenticated parity remains to be run with a disposable database.

Additional recheck: dashboard card reorder now rolls back and reports API failure; Integrations now exposes working run-due, enable/disable inbound key, and rotate-key controls. Full staged TypeScript passed. Global JSON transport was already 10 MB, sufficient for the advertised 5 MB attachment payload.
IMP-02 closed: both React import flows now accept CSV/XLS/XLSX and convert the first worksheet through the same SheetJS 0.20.3 browser build used by EJS. Loading/read failures are visible inline. Full staged TypeScript passed.
